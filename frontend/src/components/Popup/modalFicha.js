...
import { listarFicha } from '../../hooks/apiService';
import styles from './modalFicha.module.css'; // Substitua pelo caminho correto do seu arquivo CSS

const ModalFicha = ({ idColaborador, onClose }) => {
    const [ficha, setFicha] = useState(null);

    useEffect(() => {
        const fetchFicha = async () => {
            try {
                const response = await listarFicha(idColaborador);
                setFicha(response);
            } catch (error) {
            ...
            <div className={styles.modal}>
                ...
                {ficha && (
                    <div>
                        ...
                        <p>Data de Emissão da Identidade: {ficha.data_emissao_identidade}</p>
                        ...
                        <p>Data de Emissão CTPS: {ficha.ctps_data_emissao}</p>
                        <p>Número PIS: {ficha.pis_numero}</p>
                        <p>Data de Cadastro PIS: {ficha.pis_data_cadastro}</p>
                        ...
                        <p>CBO: {ficha.cbo}</p>
                        <p>Data de Admissão: {ficha.data_admissao}</p>
